 ...
 * @param [in] openMode The open mode.  One of NVS_READWRITE (default) or NVS_READONLY.
 */
void NVS::init(String name, nvs_open_mode openMode)
{
 ...
 */
String NVS::get(String key, bool isBlob)
{
	ESP_LOGD(LOG_TAG, ">> get: key: %s", key.c_str());
	...
	if (isBlob)
	{
		::nvs_get_blob(m_handle, key.c_str(), (char *)res.c_str(), &length);
	}
	else
	{
		::nvs_get_str(m_handle, key.c_str(), (char *)res.c_str(), &length);
	}
	res = String(res.c_str());

	ESP_LOGD(LOG_TAG, "<< get: outputSize: %d, %s", res.length(), res.c_str());

	return res;
} // get

int NVS::get(String key, uint32_t &value)
{
	return ::nvs_get_u32(m_handle, key.c_str(), &value);
} // get - uint32_t

int NVS::get(String key, uint8_t *result, size_t &length)
{
 ...
 */
void NVS::set(String key, String data, bool isBlob)
{
	ESP_LOGD(LOG_TAG, "key: %s, string: value=%s", key.c_str(), data.c_str());
	if (isBlob)
	{
		esp_err_t err = ::nvs_set_blob(m_handle, key.c_str(), data.c_str(), data.length());
		if (err != ESP_OK)
			ESP_LOGE(LOG_TAG, "Error: %d", err);
	}
	else
	{
		esp_err_t err = ::nvs_set_str(m_handle, key.c_str(), data.c_str());
		if (err != ESP_OK)
			ESP_LOGE(LOG_TAG, "Error: %d", err);
	}
	ESP_LOGD(LOG_TAG, "done");
} // set

void NVS::set(String key, uint32_t value)
{
...
void NVS::set(String key, uint8_t *data, size_t length)
{
